...
class client{
  ...
  async createCollection( collection ){
    try{
      let exists = await mongo.db( this._db ).listCollections({ name: collection })?.toArray()
  ...
  async math(collection, matchCondition, data){
    try{
      return await mongo.db( this._db ).collection( collection ).updateOne( matchCondition, { $inc: data, $set: { TTL: new Date() } }, { "upsert":true } )
    }catch(e){
      log.error(e, this.cache_name)
    }
  }
  ...
  async unset(collection, matchCondition, data){
    try{
      return await mongo.db( this._db ).collection( collection ).updateOne(matchCondition, { $unset: data, $set: { TTL: new Date() } })
    }catch(e){
      log.error(e, this.cache_name)
    }
  }
  ...
  watch( collection ){
    try{
      return mongo.db( this._db ).collection( collection ).watch( { fullDocument: 'updateLookup' } )
    }catch(e){
      log.error(e)
    }
  }
